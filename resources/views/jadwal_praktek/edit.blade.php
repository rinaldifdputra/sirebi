        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">Edit Jadwal Praktek</h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <div class="container mt-5">
                        <form class="form-horizontal" action="{{ route('jadwal_praktek.update', $jadwal->id) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            <div class="box-body">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="select2-container @error('jam_praktek_id') is-invalid @enderror"
                                            ...
                                            @foreach ($jam_praktek as $jam)
                                                <option value="{{ $jam->id }}"
                                                    {{ old('jam_praktek_id', $jadwal->jam_praktek_id) == $jam->id ? 'selected' : '' }}>
                                        ...
                                        <select class="select2-container @error('bidan_id') is-invalid @enderror"
                                            ...
                                            @if (Auth::user()->role == 'Admin')
                                                ...
                                                @foreach ($bidan as $bdn)
                                                    <option value="{{ $bdn->id }}"
                                                        {{ old('bidan_id', $jadwal->bidan_id) == $bdn->id ? 'selected' : '' }}>
                                                        {{ $bdn->nama_lengkap }}
                                                    </option>
                                                @endforeach
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control @error('kuota') is-invalid @enderror"
                                            id="kuota" name="kuota" placeholder="Kuota"
